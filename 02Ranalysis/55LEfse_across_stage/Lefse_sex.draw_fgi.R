...
fgi.lefse = read.table("lefse.result.fgi.tsv",sep="\t")
fgi.lefse$type = factor(typeName[fgi.lefse$type,'name'],levels = c('Leaf','Root','Rhizo.'),ordered = T)
fgi.lefse = fgi.lefse[order(fgi.lefse$Taxa),]
fgi.lefse$TaxaShort = gsub(".*\\|","",fgi.lefse$Taxa)
fgi.lefse$Group = factor(ifelse(fgi.lefse$Group=='F','Female','Male'),levels=c('Female','Male'),ordered = T)


#筛选出目和科的级别

fgi.lefse.of = fgi.lefse[grep("[o]__",fgi.lefse$TaxaShort),]

fgi.lefse.of.clean = fgi.lefse.of[c('Group',"LDA",'type',"TaxaShort")]

count.LDA = as.data.frame(tapply(fgi.lefse.of.clean$LDA, fgi.lefse.of.clean$TaxaShort, length))
mean.LDA = as.data.frame(tapply(fgi.lefse.of.clean$LDA, fgi.lefse.of.clean$TaxaShort, mean))
...
important.lefse = fgi.lefse[fgi.lefse$TaxaShort %in% important.Taxa,c("Group","LDA","Significance","type","TaxaShort")]
...
ggsave("LEfse.LDA.heat.fgiOrder10.pdf",width=60,height =45,units = "mm")
...
fgi.lefse.of = fgi.lefse[grep("[f]__",fgi.lefse$TaxaShort),]
...
ggplot(data = important.lefse)+
  ...
  theme(axis.title = element_blank(),axis.ticks = element_blank(),
        ...
        legend.ticks = element_line(colour = "#ddd",linewidth = .linewidth),
        legend.title = element_text(size = .fontsizeTitlePT))
ggsave("LEfse.LDA.heat.fgiFamily10.pdf",width=60,height =45,units = "mm")
