...
feature_table = read.table(file="../../00data/bacterial/feature_table_10clean.tsv")
...
classified_table = read.table('../../00data/bacterial/classified_table_10clean.tsv',header=TRUE, sep="\t",na.strings = "NA",comment.char = "")
...
write.table(lefse.result.frame,file = "lefse.result.bac.tsv",sep="\t",quote = F,col.names = T,row.names = T)
write.table(lefse.abund.frame,file = "lefse.abund.bac.tsv",sep="\t",quote = F,col.names = T,row.names = T)
