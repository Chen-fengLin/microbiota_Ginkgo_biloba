...
OTUCompare<- function(feature.sum,sampleMeta,main,scales = "fixed"){
  ...
  plot = ggplot(data=plotdata.use)+
    ...
    scale_x_discrete()+
    scale_y_continuous(expand = c(0,0,0.2,0),name = 'Relative abundance',
    ...
    theme(axis.ticks.x = element_blank(),
          ...
          title = element_text(size = 7.5,color="black",face = "bold"))
...
# diff.signif = fgi.diff.signif = fgi.diff[fgi.diff$p.value<=0.05,]

# 
# Root_sigOTU = diff.signif[diff.signif$type=='R' & diff.signif$period=='S1',]
# Root_sigOTU_pp = OTU.plant_pathogen[OTU.plant_pathogen %in% Root_sigOTU$OTU_id]
# 
# a=Root_sigOTU[Root_sigOTU$OTU_id %in% Root_sigOTU_pp,]
# rownames(a) = a$OTU_id
# classify_root_pp = fgi.classify[fgi.classify$OTU_id %in% Root_sigOTU_pp,]
# classify_root_pp$Great = a[classify_root_pp$OTU_id,"Great"]



OTU.plant_pathogen = rownames(fgi.function)[fgi.function$plant_pathogen>0]
plant_pathogen.res = OTUCompare.Global(fgi.feature,OTU.plant_pathogen,fgi.sampleMeta,main="All plant pathogen")
plant_pathogen.global = addsignif(plant_pathogen.res,c('ab','a','b','c','d','d',
                                                             'a','a','a','a','a','a',
                                                             'ab','a','bc','b','ac','ac'))

# Alternaria alternata 链格孢属（格孢菌目）  →黑腐病\叶枯病，雌性抗性弱
OTU.Alternaria = fgi.classify[!is.na(fgi.classify$genus) & fgi.classify$genus=='Alternaria',"OTU_id"]
Alternaria.res = OTUCompare.Global(fgi.feature,OTU.Alternaria,fgi.sampleMeta,main="Alternaria (Pleosporales)")
Alternaria.global = addsignif(Alternaria.res,c('a','a','a','a','b','b',
                                               'a','a','a','ab','ab','b',
                                               'a','a','a','a','a','a'))


# Colletotrichum gloeosporioides  炭疽菌属 → 炭疽病\叶枯病，雌性抗性弱
OTU.Colletotrichum = fgi.classify[grep("Colletotrichum",fgi.classify$species),"OTU_id"]
Colletotrichum.res = OTUCompare.Global(fgi.feature,OTU.Colletotrichum,fgi.sampleMeta,main="Colletotrichum (Glomerellales)")
Colletotrichum.global = addsignif(Colletotrichum.res,c('a','a','a','a','a','a',
                                               'a','a','a','a','a','a',
                                               'ab','a','b','ab','ab','ab'))


# Phyllosticta ginkgo 叶点霉属（葡萄座腔菌目） → 叶斑病
OTU.Phyllosticta = fgi.classify[!is.na(fgi.classify$genus) & fgi.classify$genus=='Guignardia',"OTU_id"]

Phyllosticta.res = OTUCompare.Global(fgi.feature,OTU.Phyllosticta,fgi.sampleMeta,main="Phyllosticta (Botryosphaeriales)")
Phyllosticta.global = addsignif(Phyllosticta.res,c('a','b','b','c','d','d',
                                             'a','b','ab','b','ab','ab',
                                             'a','a','a','a','a','a'))

# Pestalotiopsis ginkgo 银杏拟盘多毛孢菌 →靶斑病
OTU.Pestalotiopsis = fgi.classify[!is.na(fgi.classify$genus) & fgi.classify$genus=='Pestalotiopsis',"OTU_id"]

Pestalotiopsis.res = OTUCompare.Global(fgi.feature,OTU.Pestalotiopsis,fgi.sampleMeta,main="Pestalotiopsis (Xylariales)")
Pestalotiopsis.global = addsignif(Pestalotiopsis.res,c('a','a','a','a','a','a',
                                                   'a','a','a','a','a','a',
                                                   'a','a','a','a','a','a'))

# Fusarium oxysporum镰刀菌属(Hypocreales) →根腐病

OTU.Fusarium = fgi.classify[c(grep("Fusarium",fgi.classify$species),grep("Fusarium",fgi.classify$genus)),"OTU_id"]
Fusarium.res = OTUCompare.Global(fgi.feature,OTU.Fusarium,fgi.sampleMeta,main="Fusarium (Hypocreales)")
Fusarium.global = addsignif(Fusarium.res,c('a','a','bc','b','a','ac',
                                           'ab','a','a','a','b','b',
                                           'a','a','a','a','a','a'))
...
p<-function(plot){
  plot+theme(strip.text = element_blank())
}
ggpubr::ggarrange(p(plant_pathogen.global$plot),
                  p(Alternaria.global$plot),
                  p(Colletotrichum.global$plot),
                  p(Phyllosticta.global$plot),
                  p(Pestalotiopsis.global$plot),
                  p(Fusarium.global$plot),
                  align = "hv",nrow=3,ncol=2,labels = paste0('(',letters[1:6],')'),
                  font.label = labelFont,
                  common.legend = T,legend = "bottom")

ggsave("plant_pathogens.pdf",
