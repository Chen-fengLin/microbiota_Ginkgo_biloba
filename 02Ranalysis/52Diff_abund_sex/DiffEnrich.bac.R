...
for (type in c('L','R','S')) {
  for(stage in c('S1','S2','S3')){
    data.res = SexDiff(data = bac.alldata[,bac.sampleMeta$type==type & bac.sampleMeta$period==stage],
                                meta = bac.sampleMeta[bac.sampleMeta$type==type & bac.sampleMeta$period==stage,])
    n.signif = sum(data.res$p.value<=0.05 & apply(data.res[,c("F.Mean","M.Mean")],1,max)>=0.01)
    print(paste(type,stage,n.signif))
    write.table(data.res,file=paste0("Diff_res_bac//",type,".",stage,".sexDiff.tsv"),sep="\t",quote = FALSE)
    resData[[type]][[stage]]=data.res
  }
}

save(resData,file = "bac.diffSex.sex.Rdata")
####plot####
source("DiffEnrich.R")
library(rlang)
loadS = load("bac.diffSex.sex.Rdata")
...
##开始作图
bac.resplot = list(
  L.S1 = sexDiffPlot(res=diff.sex,type='L',stage='S1',classify=bac.classify,colorFrame=NA),
  L.S2 = sexDiffPlot(res=diff.sex,type='L',stage='S2',classify=bac.classify,colorFrame=NA),
  L.S3 = sexDiffPlot(res=diff.sex,type='L',stage='S3',classify=bac.classify,colorFrame=NA),
  R.S1 = sexDiffPlot(res=diff.sex,type='R',stage='S1',classify=bac.classify,colorFrame=NA),
  R.S2 = sexDiffPlot(res=diff.sex,type='R',stage='S2',classify=bac.classify,colorFrame=NA),
  R.S3 = sexDiffPlot(res=diff.sex,type='R',stage='S3',classify=bac.classify,colorFrame=NA),
  S.S1 = sexDiffPlot(res=diff.sex,type='S',stage='S1',classify=bac.classify,colorFrame=NA),
  S.S2 = sexDiffPlot(res=diff.sex,type='S',stage='S2',classify=bac.classify,colorFrame=NA),
  S.S3 = sexDiffPlot(res=diff.sex,type='S',stage='S3',classify=bac.classify,colorFrame=NA)
)
...
# 
ggsave(filename = "bac.L123R123S123.pdf",
...
write.table(resTable,file = "bac.TableS.tsv",sep="\t",quote = F,row.names = F)
