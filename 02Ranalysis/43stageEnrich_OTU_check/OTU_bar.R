...
OTUCompare.GroupByType <- function(feature,OTUList,sampleMeta,diff.signif){
  abundRes=NULL
  for (type in unique(diff.signif$type)) {
    diff.signif.type = diff.signif[diff.signif$type==type,]
    OTUList.type = OTUList[OTUList %in% diff.signif.type$OTU_id]
    sampleList.type = sampleMeta$sampleID[sampleMeta$type==type]
    feature.func.type = as.data.frame(t(feature[OTUList.type,sampleList.type]))
    feature.sum.type = as.data.frame(apply(feature.func.type,1,sum)/1e3)
    colnames(feature.sum.type)='abund'
    abundRes = rbind(abundRes,feature.sum.type)
  }
  
  OTUCompare(abundRes,sampleMeta)
}

OTUCompare.GroupByTypePeriod <- function(feature,OTUList,sampleMeta,diff.signif){
  abundRes=NULL
  for (type in unique(diff.signif$type)) {
    for (period in unique(diff.signif$period)){
      diff.signif.typeperiod = diff.signif[diff.signif$type==type & diff.signif$period==period,]
      OTUList.typeperiod = OTUList[OTUList %in% diff.signif.typeperiod$OTU_id]
      sampleList.typeperiod = sampleMeta$sampleID[sampleMeta$type==type & sampleMeta$period==period]
      feature.func.typeperiod = as.data.frame(t(feature[OTUList.typeperiod,sampleList.typeperiod]))
      feature.sum.typeperiod = as.data.frame(apply(feature.func.typeperiod,1,sum)/1e3)
      colnames(feature.sum.typeperiod)='abund'
      abundRes = rbind(abundRes,feature.sum.typeperiod)
    }
  }
...
#   OTUList = OTUList[OTUList %in% rownames(diff.signif)]
...
OTU.Deltaproteobacteria = bac.classify$OTU_id[which(!is.na(bac.classify$class) & bac.classify$class=='Deltaproteobacteria')]
# 
Deltaproteobacteria.res = OTUCompare.Global(bac.feature,OTU.Deltaproteobacteria,bac.sampleMeta)
Deltaproteobacteria.global = addsignif(Deltaproteobacteria.res,c('a','bd','c','bc','d','d',
                                                 'a','a','a','a','b','c',
                                                 'ac','ab','ab','a','b','c'))
Deltaproteobacteria.global
ggsave("Deltaproteobacteria.pdf",width=14,height=10,units = 'cm')
# 


OTU.Myxococcales = bac.classify$OTU_id[which(!is.na(bac.classify$order) & bac.classify$order=='Myxococcales')]
# 
Myxococcales.res = OTUCompare.Global(bac.feature,OTU.Myxococcales,bac.sampleMeta)
Myxococcales.global = addsignif(Myxococcales.res,c('a','bd','c','bc','bcd','d',
                                                                 'ab','ab','a','b','c','c',
                                                                 'a','ab','ab','ab','ab','b'))
Myxococcales.global
ggsave("Myxococcales.pdf",width=14,height=10,units = 'cm')


ggpubr::ggarrange(Deltaproteobacteria.global$plot,Myxococcales.global$plot,
                  nrow=2,ncol=1,labels = c('(a)','(b)'),align = "hv",
                  font.label = labelFont,common.legend = T,legend = "bottom")

ggsave("Deltaproteobacteria+Myxococcales.pdf",width = 18,height = 16,units = "cm")
