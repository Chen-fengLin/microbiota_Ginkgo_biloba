...
OTU.classify = read.table("../../00data/fungi/classified_table_10clean.tsv",sep="\t")
...
library(ape)
library(ggtree)
OTU.tree=read.tree("../../00data/fungi/fgi.tree")
...
source("../../public/public.R")
load(file = "fgi.01iCAMPdata.Rdata")
...
PREFIX = 'fgi'
load(paste0(PREFIX,".01iCAMPdata.Rdata"))
OTU.abundance = read.table("../../00data/fungi/feature_table_20norm.tsv")
...
taxaColor = c(Ascomycota="#E78AC3",
              Dothideomycetes="#66C2A5",
              Eurotiomycetes="#D5B484",
              Sordariomycetes="#FC8D62",
              Basidiomycota="#A6D854",
              Others="#B3B3B3")
...
fgi.plot = ggtree(bin.tree.groupByTaxa,aes(colour = group),layout = "circular",open.angle = 180,
  ...
  geom_fruit(data = bin.type.hoSAbund.Soil,geom=geom_col,
             ...
             pwidth = 0.2126632*1.327286,offset=0.2126632*1.053999*1.1*1.327286*1.327286,
  ...
  geom_fruit(data = bin.type.hoSAbund.Root,geom=geom_col,
             ...
             pwidth = 0.1338757*1.327286,offset=0.1338757*1.053999*1.1*1.327286*1.327286,
             axis.params = list(axis="xy",
                                text.size=.fontsize/3/ggplot2::.pt,
                                line.size=.linewidth/5,
                                limits=c(-45,25)))+
  ...
  geom_fruit(data = bin.type.hoSAbund.Leaf,geom=geom_col,
             ...
             pwidth = 0.2857895*1.327286,offset=0.2857895*1.053999*1.1*1.327286*1.327286,
             axis.params = list(axis="xy",
                                text.size=.fontsize/3/ggplot2::.pt,
                                line.size=.linewidth/5,
                                limits=c(-75,95)))+
...
save(fgi.plot,file="fgi.19.plot.Rdata")
...
typesexMean.HoS = read.table(paste0(PREFIX,".06-2typesex.bin.hoS.tsv"))
# 
...
fgi.abund.plot = abund.plot;
fgi.hos.plot=hoS.plot;
fgi.sexDiff=sexDiff.plot
save(fgi.abund.plot,fgi.hos.plot,fgi.sexDiff,
